...
export default async function ReleaseCollectionTable({ searchParams, isCollection }) {
    ...
    if (isCollection) {
        ...
        // querying user data 
        const { data: CollectionData, error: CollectionError } = (await supabase.from('user_data').select().in('r_id', releaseIDs).eq('user_id', user.id).range(parseInt(page) * 25, (parseInt(page) + 1) * 25 - 1))
