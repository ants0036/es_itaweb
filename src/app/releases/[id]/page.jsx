  ...
  async function Incrementer() {
    ...
    // if there is currently an idol selected;
      ...
      const { data: countData, error: countError } = await supabase
        ...
        .select("qty")
        .eq("r_id", params.id)
        .eq("i_id", searchParams.i_id)
        .eq("user_id", user.id)
        .single();
      // fetch the selected idol's information.
      const { data: selectedIdolData, error: idolError } = await supabase
        .from("Idols")
        .select()
        .eq("id", searchParams.i_id)
        .single();
      ...
      return (
        <div>
          <p>
            {" "}
            You have selected {selectedIdolData.f_name}. You currently own{" "}
            {TotalOwned} of this idol.{" "}
          </p>
          <IncrementerForm
            i_id={searchParams.i_id}
            r_id={params.id}
      ...
      return <p> Please select an Idol. </p>;
    }
  }
        ...
        <div>
          ...
          <div className="flex flex-wrap pr-10">
            {mergeData.map(async (val, key) => {
              ...
              if (val.i_id2 != null) {
                const { data: idolData2, error: idolError2 } = await supabase
                  .from("Idols")
                  .select()
                  .eq("id", val.i_id2)
                  .single();
                ...
                if (val.i_id3 != null) {
                  const { data: idolData3, error: idolError3 } = await supabase
                    .from("Idols")
                    .select()
                    .eq("id", val.i_id3)
                    .single();
